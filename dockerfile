FROM itdoginfo/openwrt-sdk-ipk:24.10.3

ARG VERSION=0.1
ENV VERSION=v${VERSION}

# Копируем свои пакеты
COPY update-podkop          /builder/package/update-podkop
COPY luci-app-update-podkop /builder/package/luci-app-update-podkop

WORKDIR /builder

# ДЕЛАЕМ ТОЛЬКО ТАК — иначе в itdoginfo образе ничего не работает
RUN echo "src/gitrepo packages https://git.openwrt.org/feed/packages.git"    >  feeds.conf && \
    echo "src/gitrepo luci     https://git.openwrt.org/feed/luci.git"        >> feeds.conf && \
    echo "src/gitrepo routing  https://git.openwrt.org/feed/routing.git"    >> feeds.conf && \
    echo "src/gitrepo telephony https://git.openwrt.org/feed/telephony.git" >> feeds.conf && \
    echo "src-link custom /builder/package"                                 >> feeds.conf && \
    ./scripts/feeds update -a && \
    ./scripts/feeds install -a && \
    make defconfig && \
    make package/update-podkop/compile          V=sc -j$(nproc) && \
    make package/luci-app-update-podkop/compile V=sc -j$(nproc)

# Одна строка CMD в JSON-формате (без warning)
CMD ["/bin/sh","-c","find bin -name '*update-podkop*.ipk' -o -name '*luci-app-update-podkop*.ipk' -exec cp {} /out/ \\;"]